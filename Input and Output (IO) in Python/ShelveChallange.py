# Modify the program from the Second Dictionary challange of lecure 56
# to use shelves instead of dictionaries.
#
# Do this by creating two programs. CaveInitialise.py should create the two
# shelves (locations and vocabulary) with the appropriate keys and values.
#
# CaveGame.py will then use the two shelves instead of dictionaries.
# Apart from opening and closing the shelves, CaveGame will need only
# two changes to the actual code - remember that shelf keys MUST be strings!
#
# Just to be clear, CaveGame.py will contain the code from line 45, everything
# before that (modified to use shelves) will be in CaveInitialise.py
